...
const ShieldIcon = ({ className = 'w-8 h-8 text-orange-600' }: IconProps) => (
...
const ChartIcon = ({ className = 'w-4 h-4' }: IconProps) => (
  <svg className={className} fill="currentColor" viewBox="0 0 20 20">
...
export default function AdminNavbar({ onToggleSidebar }: NavbarProps) {
  ...
  const demoAdmin = {
    ...
    email: admin?.email || "admin@techservice.com",
    id: admin?.id || "ADM001",
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden lg:flex flex-1 max-w-lg mx-4 lg:mx-8">
            <div className="relative w-full">
              ...
              <input
                type="text"
                placeholder="Search requests, customers, employees..."
                value={searchTerm}
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden sm:block text-left min-w-0">
                  <div className="text-sm font-medium text-gray-900 truncate">{demoAdmin.name}</div>
                  <div className="text-xs text-gray-500 truncate">{demoAdmin.email}</div>
                </div>
                <ChevronDownIcon />
              </button>
              ...
              {isProfileDropdownOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg py-2 z-50 border border-gray-200">
                  ...
                  <div className="px-4 py-3 border-b border-gray-100">
                    <div className="flex items-center space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-gray-900 truncate">{demoAdmin.name}</div>
                        <div className="text-xs text-gray-500 truncate">{demoAdmin.email}</div>
                        <div className="text-xs text-gray-500 truncate">ID: {demoAdmin.id}</div>
                      </div>
                    </div>
                    <div className="mt-3 flex items-center justify-between">
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(demoAdmin.status)}`}>
                        {getStatusText(demoAdmin.status)}
                      </span>
                      <div className="flex items-center text-xs text-gray-500">
                        <StarIcon className="w-3 h-3 mr-1 text-orange-500" />
                        {demoAdmin.role}
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Admin Stats */}
                  <div className="px-4 py-3 border-b border-gray-100">
                    <div className="text-xs font-medium text-gray-700 mb-2">Today's Overview</div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="text-center">
                        <div className="text-lg font-bold text-orange-600">{demoAdmin.todayTasks}</div>
                        <div className="text-xs text-gray-500">Total Tasks</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-bold text-green-600">{demoAdmin.completedToday}</div>
                        <div className="text-xs text-gray-500">Completed</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-bold text-blue-600">{notificationCount}</div>
                  ...
                  <div className="px-4 py-2 border-b border-gray-100">
                    ...
                    <div className="flex items-center text-sm font-medium text-gray-900">
                      ...
                      {demoAdmin.systemHealth}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">Permissions: {demoAdmin.permissions}</div>
                  </div>
                  ...
                  <div className="py-2">
                    ...
                    <button
                      ...
                      className="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                    >
                      <ChartIcon className="w-4 h-4 mr-3" />
                      System Analytics
                    </button>
                    <button
                      onClick={() => {
