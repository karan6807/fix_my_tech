...
export default function EmployeeSidebar({ isCollapsed, onToggle }: SidebarProps) {
  const router = useRouter();
  const pathname = usePathname();
  const [currentPath, setCurrentPath] = useState('/employee-frontend/dashboard');
  ...
  const navItems = [
    { href: '/employee-frontend/dashboard', label: 'Dashboard', icon: <DashboardIcon /> },
    { href: '/employee-frontend/my-tasks', label: 'My Tasks', icon: <ServicesIcon /> },
    { href: '/employee-frontend/schedule', label: 'Schedule', icon: <UsersIcon /> },
    { href: '/employee-frontend/reports', label: 'Reports', icon: <AnalyticsIcon /> },
    { href: '/employee-frontend/profile', label: 'Profile', icon: <SettingsIcon /> },
  ];

  const isActive = (href: string) => {
    if (href === '/employee-frontend/dashboard') {
      return currentPath === '/employee-frontend/dashboard';
    }
    ...
    <div className={`bg-white border-r border-gray-200 h-full flex flex-col transition-all duration-300 ${
      ...
      <div className="lg:hidden flex items-center justify-between p-4 border-b border-gray-200">
        <div className="flex items-center">
          ...
          <span className="ml-3 text-lg font-bold text-gray-900">Employee Portal</span>
        </div>
        <button
          onClick={onToggle}
      ...
      <div className="hidden lg:flex items-center p-4 border-b border-gray-200">
        {!isCollapsed && (
          <div className="flex items-center">
            ...
            <span className="ml-3 text-lg font-bold text-gray-900">Employee</span>
          </div>
        )}
        {isCollapsed && (
